...
class GalaxyAnalysis:
    ...
    def setMassSubgroups(self, list_mass_groups : list[ListGalaxyGroup]):
        self.filtered_gt13_ls13p5_list_of_galaxy_groups = list_mass_groups[0]
        self.filtered_gt13p5_ls14_list_of_galaxy_groups = list_mass_groups[1]
        self.filtered_gt14_ls14p5_list_of_galaxy_groups = list_mass_groups[2]
        self.filtered_gt14p5_ls15_list_of_galaxy_groups = list_mass_groups[3]
        self.filtered_gt15_list_of_galaxy_groups = list_mass_groups[4]
    ...
    def pairwisePolarDifferencePlot(self, list_of_galaxy_group : ListGalaxyGroup, plot_dirc : str = None):
        ...
        pairwise_polar_differences_binned, polar_bin_centers, pairwise_polar_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
    ...
    def redVsBlueDistributionPlot(self, list_of_galaxy_groups : ListGalaxyGroup = None, plot_dirc : str = None, list_photometrics : list[tuple[float, float, float, float, float, float, float, float]] = None):
        ...
        if list_of_galaxy_groups is not None:
            for galaxy_group in list_of_galaxy_groups.getAllGalaxyGroups():
                for subhalo in galaxy_group.getSubhalos():
                    ...
                    gMr_values_raw.append(g_r_color*-1)
        ...
        gMr_values = np.array(gMr_values_raw).reshape(-1, 1)
        ...
        # Generate x values
        x = np.linspace(gMr_values.min(), gMr_values.max(), 1000)
        ...
        if intersection_idx.size > 0:
            intersection_point = x[intersection_idx[0]]
            intersection_point_valid = True
        else:
            intersection_point = 0.65
        ...
        color_plotter.line_plot(
            x,
            pdf1,
            color='blue',
            ax=color_ax,
            label=f'Gaussian Fit (μ={means[0]:.2f}, σ={stds[0]:.2f})',
            # include_legend=True,
        )
        color_plotter.line_plot(
            x,
            ...
            color='red',
            ax=color_ax,
            label=f'Gaussian Fit (μ={means[1]:.2f}, σ={stds[1]:.2f})',
            # include_legend=True,
        )
        color_ax.axvline(
            intersection_point,
        ...
        color_plotter.add_text_box(
            color_ax, 
            f"Means (μ) = {means[0]:.2f}, {means[1]:.2f}\nStandard Deviation (σ) = {stds[0]:.2f}, {stds[1]:.2f}\nIntersection Point = {intersection_point:.2f}, {'T' if intersection_point_valid else 'F'}", 
            loc='bottom center'
        )
        ...
        return intersection_point, intersection_point_valid, color_plotter, color_fig, color_ax
    ...
    def redVsBluePairwisePlot(self, list_of_galaxy_groups : ListGalaxyGroup = None, plot_dirc : str = None):
        ...
        pairwise_polar_differences_red, polar_bin_centers_red, pairwise_polar_red_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_red, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
        pairwise_polar_differences_blue, polar_bin_centers_blue, pairwise_polar_blue_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_blue, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
        ...
        prob_polar_red_blue_plotter.scatter_plot(
            ...
            label = f"Red Galaxies (g-r) ≥ {intersectionPoint:.2f}",
    ...
    def member150v50Plot(self, list_of_galaxy_groups : ListGalaxyGroup = None, plot_dirc : str = None):
        ...
        pairwise_polar_differences_GT150, polar_bin_centers_GT150, pairwise_polar_GT150_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_GT150, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
        pairwise_polar_differences_LT50, polar_bin_centers_LT50, pairwise_polar_LT50_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_LT50, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
    ...
    def memberL35vG65Plot(self, list_of_galaxy_groups : ListGalaxyGroup = None, plot_dirc : str = None):
        ...
        pairwise_polar_differences_LT35R200, polar_bin_centers_LT35R200, pairwise_polar_LT35R200_errorbars = filtered_LT35R200_list_of_galaxy_groups.get_histogram_bins(list_pairwise_polar_differences_LT35R200, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
        pairwise_polar_differences_GT65R200, polar_bin_centers_GT65R200, pairwise_polar_GT65R200_errorbars = filtered_GT65R200_list_of_galaxy_groups.get_histogram_bins(list_pairwise_polar_differences_GT65R200, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
    ...
    def centralFoFDistanceOffsets(self, plot_dirc : str = None):
        ...
        pairwise_polar_differences_13_14, polar_bin_centers_13_14, pairwise_polar_13_14_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_13_14, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
        
        pairwise_polar_differences_gt14, polar_bin_centers_gt14, pairwise_polar_gt14_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_gt14, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
    ...
    def probabilityDistributionOf5MassGroups(self, plot_dirc : str = None):
        ...
        prob_polar_mass_plotter_by_mass, prob_polar_mass_fig_by_mass, prob_polar_mass_ax_by_mass = self.plot_pairwise_polar_by_mass_bins_and_centralMassive_status([(self.filtered_gt13_ls13p5_list_of_galaxy_groups, '1e13<$M_{{200}}$<1e13.5 Msun'),
                                                                    (self.filtered_gt13p5_ls14_list_of_galaxy_groups, '1e13.5<$M_{{200}}$<1e14 Msun'),
                                                                    (self.filtered_gt14_ls14p5_list_of_galaxy_groups, '1e14<$M_{{200}}$<1e14.5 Msun'),
                                                                    (self.filtered_gt14p5_ls15_list_of_galaxy_groups, '1e14.5<$M_{{200}}$<1e15 Msun'),
                                                                    (self.filtered_gt15_list_of_galaxy_groups, '$M_{{200}}$>1e15 Msun')], plot_dirc=plot_dirc)
       
        prob_polar_mass_plotter_by_mass.save_figure(prob_polar_mass_fig_by_mass, self.scratchPlotDirc + f'/pairwise_polar_difference_allMasses_{self.plotIdentifier}_{self.plotEndingFormat}') if plot_dirc is None else prob_polar_mass_plotter_by_mass.save_figure(prob_polar_mass_fig_by_mass, plot_dirc + f'/pairwise_polar_difference_allMasses_{self.plotIdentifier}_{self.plotEndingFormat}')
        ...
        # prob_polar_mass_ax_by_mass.errorbar(polar_bin_centers, pairwise_polar_differences_binned, yerr=pairwise_polar_errorbars, fmt='o', label='All Galaxy Groups', color='black', markersize=3, alpha=0.5)
        # prob_polar_mass_ax_by_mass.legend()
        # prob_polar_mass_plotter_by_mass.save_figure(prob_polar_mass_fig_by_mass, self.scratchPlotDirc + f'/pairwise_polar_difference_allMasses_{self.plotIdentifier}_overlay{self.plotEndingFormat}') if plot_dirc is None else prob_polar_mass_plotter_by_mass.save_figure(prob_polar_mass_fig_by_mass, plot_dirc + f'/pairwise_polar_difference_allMasses_{self.plotIdentifier}_overlay{self.plotEndingFormat}')
    ...
    def plot_pairwise_polar_by_mass_bins_and_centralMassive_status(self, listGG : list[tuple[ListGalaxyGroup, str]], plot_dirc : str = None): #mass_filtered_list : ListGalaxyGroup, mass_bin_label):
        ...
        for i, (mass_filtered_list, mass_bin_label) in enumerate(listGG):
            ...
            pairwise_polar_differences_notCentralMassive, polar_bin_centers_notCentralMassive, pairwise_polar_notCentralMassive_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_notCentralMassive, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
            pairwise_polar_differences_total, polar_bin_centers_total, pairwise_polar_total_errorbars = ListGalaxyGroup.get_histogram_bins(list_pairwise_polar_differences_total, bins = np.arange(0, 185, 10), errorbarType=self.generalErrorbar, normalize_to_one=True)
    ...
    @staticmethod
    def getPercentOfClustersAboveRandomMRLCL(listGG : ListGalaxyGroup = None, percentageMRL:float = 0.99*100, random_mrl_values:list[list[float]] = None, MRL_values = None, numsamples : int = 1000) -> float:
        # Accept either a percentile in [0, 100] (e.g. 95) or a fraction in (0, 1] (e.g. 0.95).
        # Internally we always work in percentile units.
        if 0 < percentageMRL <= 1:
            print(f"Warning: percentageMRL value {percentageMRL} is in the range (0, 1]. Interpreting this as a fraction and converting to percentage units by multiplying by 100.")
            percentageMRL *= 100
        if MRL_values is None:
            if listGG is None:
                raise ValueError("Must provide either MRL_values or listGG to compute MRL values.")
        ...
        if random_mrl_values is None:
            ...
            random_MRL_values = listGG.compute_MRL_random_distribution_curves_for_LGG(parallelize=False, num_samples=numsamples) # a list of lists, outer list is number of galaxy groups, inner list is the random MRL values for that galaxy group (3000 since list of rx, ry, rz 1000 times each)
        else:
            random_MRL_values = random_mrl_values
        fraction_above_percentile, count_less_than_percentile, total_count, subhaloIndexInconsistentWithRandom = GalaxyAnalysis.calculate_fraction_less_than_percentile(MRL_values, random_MRL_values, percentageMRL)
        print(f"Fraction of MRL values below the {percentageMRL}th percentile of random MRL values: {fraction_above_percentile:.4f} ({count_less_than_percentile}/{total_count})")
        return (fraction_above_percentile, count_less_than_percentile, total_count, subhaloIndexInconsistentWithRandom), MRL_values, random_MRL_values

    def overlayMRLAndMRLRandom(self, listGG : list[tuple[ListGalaxyGroup, str]], plot_dirc : str = None, filename:str='', percentageMRL:float = 0.99*100, oneaxis:bool=False, savefig:bool=True, getInconsistentSubhalos:bool=False):
        ...
        text_box_count_by_axis: dict[int, int] = {}
        ...
        for i, (listGalaxyGroup, label) in enumerate(listGG):
            ...
            print(f"running label with list: {len(listGalaxyGroup.getAllGalaxyGroups())}")
            (fraction_less_than_nth_percentile, count_less_than_percentile, total_count, subhaloIndexInconsistentWithRandom), MRL_values, random_MRL_values = GalaxyAnalysis.getPercentOfClustersAboveRandomMRLCL(listGalaxyGroup, percentageMRL=percentageMRL) # also prints the fraction of MRL values below the given percentile of random MRL values
            # print((fraction_less_than_nth_percentile, count_less_than_percentile, total_count, subhaloIndexInconsistentWithRandom), MRL_values, random_MRL_values)
            if not MRL_values or len(MRL_values) == 0:
                print(f"Skipping {label}: No MRL values to plot.")
                continue
            MRL_binned, MRL_bin_centers, MRL_errorbars = ListGalaxyGroup.get_histogram_bins(MRL_values, binsize=0.05, binLow=0, binHigh=1, errorbarType='poisson', density=True)
            ...
            random_MRL_bins, random_MRL_bin_centers, random_MRL_errorbars = ListGalaxyGroup.get_histogram_bins(random_MRL_values, binsize=0.05, binLow=0, binHigh=1, errorbarType='poisson', density=True)
            ...
            # print(f"{percentageMRL}th percentile MRL: {percentile_MRL}")
            # print(f"Overall number of MRL values above {percentageMRL}th percentile: {np.sum(np.array(MRL_values) > percentile_MRL)} out of {len(MRL_values)}")
            ...
            overlayAxToPlot.text(
                0.5,
                text_y,
                text,
                transform=overlayAxToPlot.transAxes,
                fontsize=8,
                ha='center',
                va='center',
    ...
    def M200DistributionPlots(self, listGG : list[tuple[ListGalaxyGroup, str]], plot_dirc : str = None):
        ...
        M200_fig, M200_ax = M200_plotter.create_figure(ncols=len(listGG), nrows=1, figsize=(8*len(listGG), 6) if len(listGG) > 1 else M200_plotter.create_figure())
        for i, (listGalaxyGroup, label) in enumerate(listGG):
            M200_values = []
            ...
            if len(M200_values) <= 1:
                continue
            # M200_bins, M200_bin_edges, M200_errorbars = ListGalaxyGroup.get_histogram_bins(M200_values, bins='auto', errorbarType='poisson')
    ...
    def HighMRLPlots(self, plot_dirc : str = None):
        ...
        high_MRL_pairwise_polar_bins, high_MRL_pairwise_polar_bin_centers, high_MRL_pairwise_polar_errorbars = ListGalaxyGroup.get_histogram_bins(high_MRL_pairwise_polar_differences, bins = np.arange(0, 185, 10), errorbarType='poisson', normalize_to_one=True)
        ...
        high_MRL_pairwise_polar_bins_inner, high_MRL_pairwise_polar_bin_centers_inner, high_MRL_pairwise_polar_errorbars_inner = ListGalaxyGroup.get_histogram_bins(high_MRL_pairwise_polar_differences_inner, bins = np.arange(0, 185, 10), errorbarType='poisson', normalize_to_one=True)
        ...
        high_MRL_pairwise_polar_bins_outer, high_MRL_pairwise_polar_bin_centers_outer, high_MRL_pairwise_polar_errorbars_outer = ListGalaxyGroup.get_histogram_bins(high_MRL_pairwise_polar_differences_outer, bins = np.arange(0, 185, 10), errorbarType='poisson', normalize_to_one=True)
        high_MRL_polar_plotter.scatter_plot(
    ...
    def get_satellite_join_time(self, listGG:list[tuple[ListGalaxyGroup, str]], rewrite:Optional[bool] = None, saveUpdateToHdf5:bool = False):
        #Box boundary 
        # L=75000. #kpc
        # halfbox=L/2.   
        # h=0.6774 
            ...
            for gg in listGalaxyGroup.getAllGalaxyGroups():
                ...
                for i, subhalo in enumerate(gg.getSatelliteSubhalos()):
                    if central_subhalo is not None:
                        ...
                        processedJoinTimes.append(subhalo.getJoiningRedshift())
                        # if i == 0:
                            # print(f"jointime: {join_time_info[0]}")
        print(f"\nFinished processing all satellites. Total processed: {len(processedSatelliteIds)}, expected: {totalSatellites}. Satellites without merger tree: {len(np.where(np.isnan(processedJoinTimes)))}, with non nan join times: {len(np.where(np.isfinite(processedJoinTimes)))}/{len(processedJoinTimes)}")
    ...
    def plot_joining_redshift_distribution_by_mass_bins(self, listGG : list[tuple[ListGalaxyGroup, str]]):
        ...
        for listGalaxyGroup, label in listGG:
            ...
            joining_redshift_plotter.histogram(
                joining_redshifts,
    ...
    def overlay_polar_pairwise_across_redshifts(self, listRedshiftGG:list[list[tuple[ListGalaxyGroup, str]]], plot_dirc:str = None, polar_plotter=None, polar_fig=None, polar_ax=None, typePlot:str = 'polar', plotRows:int = 0, plotCols:int = 0, oneaxis=False, include_metrics:bool=False):
        ...
        for redshift_index, listGG in enumerate(listRedshiftGG):
            for i, (listGalaxyGroup, label) in enumerate(listGG):
                ...
                bin_values, bin_centers, bin_errorbars = ListGalaxyGroup.get_histogram_bins(rawValues, bins=bin, errorbarType=self.generalErrorbar, normalize_to_one=True)
